...
export const Guide: React.FC<GuideProps> = ({ onOpenCatalog, onOpenDesigns }) => {
  ...
  }, [copy.hero.description, isArabic, language]);

  const verifiedLabel = isArabic ? 'مؤكد بهذه النسخة' : 'Verified in this build';
  const pendingLabel = isArabic ? 'بعده يحتاج تأكيد' : 'Requires external confirmation';
  const bestForLabel = isArabic ? 'يناسب عادةً' : 'Commonly suited to';
  const tradeoffLabel = isArabic ? 'شنو لازم تنتبه له' : 'Important trade-off';
  const eyebrowClass = isArabic
    ? 'text-[11px] font-black leading-6 text-accent'
    : 'text-[10px] font-black uppercase tracking-[0.28em] text-accent';
  const lightEyebrowClass = isArabic
    ? 'text-[11px] font-black leading-6 text-white/65'
    : 'text-[10px] font-black uppercase tracking-[0.28em] text-white/65';
  const sectionHeadingClass = isArabic
    ? 'mt-5 text-4xl font-black leading-[1.28] tracking-[-0.025em] sm:text-6xl'
    : 'mt-5 font-display text-4xl font-black uppercase leading-[.92] tracking-[-0.055em] sm:text-6xl';
  ...
  return (
    <div className="overflow-x-clip bg-paper text-ink dark:bg-background dark:text-primary">
      <section className="relative isolate overflow-hidden px-4 pb-16 pt-28 sm:px-6 sm:pb-20 sm:pt-36 lg:px-8">
          ...
          <div className="mt-10 flex flex-col gap-3 sm:flex-row">
            <Magnetic strength={0.18}>
              <button
                ...
                className={`inline-flex min-h-14 w-full items-center justify-center gap-3 rounded-full bg-ink px-7 text-white shadow-[0_18px_45px_rgba(0,0,0,.18)] transition-transform hover:-translate-y-0.5 dark:bg-paper dark:text-ink sm:w-auto ${actionClass}`}
              >
                {localized(copy.hero.primaryCta)}
                <span className={isArabic ? 'rotate-180' : ''} aria-hidden="true">
                  →
                </span>
              </button>
            </Magnetic>
            <button
              ...
              className={`inline-flex min-h-14 items-center justify-center rounded-full border border-black/15 bg-white/40 px-7 transition-colors hover:border-black hover:bg-white dark:border-white/15 dark:bg-white/[0.035] dark:hover:border-white dark:hover:bg-white/[0.07] ${actionClass}`}
            >
      ...
      <nav
        aria-label={localized(copy.navigation.label)}
        className="no-scrollbar border-y border-black/10 bg-white/45 px-4 dark:border-white/10 dark:bg-white/[0.025] sm:px-6 lg:px-8"
      >
        <div className="mx-auto flex max-w-[1450px] gap-2 overflow-x-auto py-3">
      ...
      <section
        ...
        className="scroll-mt-28 border-b border-black/10 bg-ink px-4 py-20 text-primary dark:border-white/10 sm:px-6 sm:py-28 lg:px-8"
      >
        <div className="mx-auto max-w-[1450px]">
          ...
          <div className="mt-12 grid gap-4 md:grid-cols-2">
            {copy.expectations.items.map((item) => (
              <article
                key={item.id}
                className="flex min-w-0 items-start gap-4 rounded-[1.75rem] border border-white/10 bg-white/[0.045] p-6 sm:p-7"
              >
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-3 break-words text-sm leading-7 text-white/52" dir="auto">
        ...
        <div className="mx-auto grid max-w-[1450px] gap-10 lg:grid-cols-[.8fr_1.2fr] lg:gap-20">
          ...
          <ol className="divide-y divide-black/10 border-y border-black/10 dark:divide-white/10 dark:border-white/10">
            {copy.sizing.steps.map((step) => (
              <li key={step.id} className="grid min-w-0 gap-4 py-7 sm:grid-cols-[64px_1fr] sm:py-8">
                <bdi className="technical-ltr font-mono text-sm font-black text-accent" dir="ltr">
                  {step.index}
                </bdi>
                <div className="min-w-0">
      ...
      <section
        ...
        className="scroll-mt-28 border-y border-black/10 bg-[#e9e3d8] px-4 py-20 dark:border-white/10 dark:bg-[#0e0e0e] sm:px-6 sm:py-28 lg:px-8"
      >
        <div className="mx-auto max-w-[1450px]">
      ...
      <section id="preparation" data-guide-section="preparation" className="scroll-mt-28 px-4 py-20 sm:px-6 sm:py-28 lg:px-8">
        <div className="mx-auto grid max-w-[1450px] gap-10 lg:grid-cols-[.72fr_1.28fr] lg:gap-20">
          <div className="min-w-0">
            ...
            <h2 className={sectionHeadingClass}>{localized(copy.preparation.title)}</h2>
          </div>
      ...
      <section
        id="policies"
        data-guide-section="policies"
        className="scroll-mt-28 border-y border-black/10 bg-ink px-4 py-20 text-primary dark:border-white/10 sm:px-6 sm:py-28 lg:px-8"
      >
        <div className="mx-auto max-w-[1450px]">
          <p className={eyebrowClass}>{localized(copy.policies.eyebrow)}</p>
          <h2 className={sectionHeadingClass}>{localized(copy.policies.title)}</h2>
          <div className="mt-12 grid gap-px overflow-hidden rounded-[2rem] border border-white/10 bg-white/10 sm:grid-cols-2 lg:grid-cols-3">
            {copy.policies.items.map((policy, index) => (
              <article key={policy.id} className="min-w-0 bg-ink p-6 sm:p-8">
      ...
      <section id="faq" data-guide-section="faq" className="scroll-mt-28 px-4 py-20 sm:px-6 sm:py-28 lg:px-8">
        <div className="mx-auto grid max-w-[1450px] gap-10 lg:grid-cols-[.72fr_1.28fr] lg:gap-20">
          <div className="min-w-0">
            ...
            <h2 className={sectionHeadingClass}>{localized(copy.faq.title)}</h2>
          </div>
          <div className="min-w-0 divide-y divide-black/10 border-y border-black/10 dark:divide-white/10 dark:border-white/10">
            {copy.faq.items.map((item, index) => (
              <details key={item.id} className="group min-w-0 py-1">
                <summary className="flex min-w-0 cursor-pointer list-none items-center gap-4 py-6 sm:gap-5 [&::-webkit-details-marker]:hidden">
                  ...
                  <span
                    className="grid h-8 w-8 shrink-0 place-items-center rounded-full border border-black/10 text-lg transition-transform group-open:rotate-45 dark:border-white/10"
                    aria-hidden="true"
                  >
                    +
                  </span>
      ...
      <section className="relative overflow-hidden bg-accent px-4 py-20 text-white sm:px-6 sm:py-28 lg:px-8">
        ...
        <div className="relative mx-auto grid max-w-[1450px] gap-10 lg:grid-cols-[1fr_auto] lg:items-end">
          <div className="min-w-0">
            ...
            <p className="mt-7 max-w-2xl break-words text-base leading-8 text-white/72" dir="auto">
          ...
          <Magnetic strength={0.18}>
            <button
              type="button"
              onClick={onOpenCatalog}
              className={`inline-flex min-h-16 items-center justify-center rounded-full bg-white px-8 text-black shadow-[0_20px_55px_rgba(80,10,4,.28)] transition-transform hover:-translate-y-1 ${actionClass}`}
            >
