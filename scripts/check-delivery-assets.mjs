...
requirePattern(index, /rel="manifest" href="\/manifest\.webmanifest"/, 'index.html must link the web manifest');
...
requirePattern(index, /name="twitter:card" content="summary_large_image"/, 'index.html must contain Twitter card metadata');
requirePattern(index, /name="bgd:indexing"/, 'index.html must expose the build indexing mode');

requirePattern(serviceWorker, /bgd-ink-shell-/, 'the service worker must use a versioned cache namespace');
requirePattern(serviceWorker, /SKIP_WAITING/, 'the service worker must support explicit update activation');
requirePattern(serviceWorker, /clients\.claim\(\)/, 'the service worker must claim clients after activation');
requirePattern(serviceWorker, /networkFirstNavigation/, 'the service worker must use network-first navigation');
requirePattern(serviceWorker, /cacheFirstAsset/, 'the service worker must cache visited static assets');
...
if (manifest.orientation !== 'any') failures.push('the manifest must allow phone and tablet rotation');
...
if (indexable) {
  requirePattern(index, /rel="canonical" href="https?:\/\//, 'an indexable build must have an absolute canonical URL');
  requirePattern(robots, /Allow:\s*\//, 'an indexable build must allow public crawling');
  requirePattern(robots, /Disallow:\s*\/studio\//, 'browser-local Studio routes must stay excluded');
  requirePattern(robots, /Sitemap:\s*https?:\/\//, 'an indexable build must publish its sitemap URL');
