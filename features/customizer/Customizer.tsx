...
export const Customizer: React.FC<CustomizerProps> = ({
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([draft, products]) => {
        ...
        const restoredActiveLayer = draft.decals.some(
        ...
        setSelectedColor(
          matchingProduct.colors.includes(draft.color)
            ? draft.color
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > MAX_ARTWORK_BYTES) {
      showToast(
        language === 'ar' ? 'حجم الصورة أكبر من 5 ميغابايت.' : 'Artwork must be 5 MB or smaller.',
        'error',
      );
      return;
    }
    ...
    try {
      ...
      showToast(
        language === 'ar' ? 'تم حفظ الصورة وإضافتها للمسودة.' : 'Artwork saved and added to the draft.',
        'success',
      );
    } catch {
      showToast(
  ...
  const handleRemoveDecal = (id: string) => {
    ...
    void queueDraftSave(snapshot)
      .then(() =>
        removedLayer?.assetId ? deleteArtworkAsset(removedLayer.assetId) : undefined,
      )
      .catch(() => undefined);
